...
<input id="permanentCityId" type="hidden"/>
<input id="permanentDistrictId" type="hidden"/>
<input id="permanentVillageId" type="hidden"/>
<input id="permanentAddress" type="hidden"/>
<script type="text/javascript">
    ...
    $("#movingToDateEnd").change(function ($event) {
        var movingToDateEnd = this.value;
        var jsonData = {};
        jsonData["id"] = $('#PageId').val();
        jsonData["action"] = "doc";
        lib.post({
            ...
            complete: function (response) {
                renderTableNhanKhauBienDong(response.responseJSON, movingToDateEnd);
    ...
    */
    ...
    $(function () {
        ...
        $("#cldrFromBirthday").datepicker({autoclose: true, todayHighlight: true}).on('changeDate', function (e) {
                $(document).pathValue({
                    ngaySinhChuHoTuNgay: e.format(0, "dd/mm/yyyy")
                });
            }
        );

        $('#ngaySinhChuHoTuNgay').on('change', function () {
            var result =  $(fromThongTinHoNhanKhau).validation({
                checkBirthdate: {
                    ngaySinhChuHoTuNgay: 'Ngày không đúng định dạng',
                }
            ...
            if (!result.isValid){
                ...
                if (this.value.length === 10) {
                    ...
                    $("#cldrFromBirthday").datepicker('setDate', new Date(dateParts[2], dateParts[1] - 1, dateParts[0]));
                }
                if (this.value.length === 0){
                    $("#cldrFromBirthday").datepicker('setDate', null);
                }
            }
        });

        $("#cldrToBirthday").datepicker({autoclose: true, todayHighlight: true}).on('changeDate', function (e) {
                $(document).pathValue({
                    ngaySinhChuHoDenNgay: e.format(0, "dd/mm/yyyy")
                });
            }
        );

        $('#ngaySinhChuHoDenNgay').on('change', function () {
            var result =  $(fromThongTinHoNhanKhau).validation({
                checkBirthdate: {
                    ngaySinhChuHoDenNgay: 'Ngày không đúng định dạng'
                }
            ...
            if (!result.isValid) {
                ...
                if (this.value.length === 10) {
                    var dateParts = this.value.split("/");
                    $("#cldrToBirthday").datepicker('setDate', new Date(dateParts[2], dateParts[1] - 1, dateParts[0]));
                }
                if (this.value.length === 0){
                    $("#cldrToBirthday").datepicker('setDate', null);
                }
            }

        });

        $("#cldrCitiFromBirthday").datepicker({autoclose: true, todayHighlight: true}).on('changeDate', function (e) {
                $(document).pathValue({
                    ngaySinhThanhVienTuNgay: e.format(0, "dd/mm/yyyy")
                });
            }
        );

        $('#ngaySinhThanhVienTuNgay').on('change', function () {
            var result =  $(fromThongTinHoNhanKhau).validation({
                checkBirthdate: {
                    ngaySinhThanhVienTuNgay: 'Ngày không đúng định dạng',
                }
            ...
            if (!result.isValid){
                ...
                if (this.value.length === 10) {
                    var dateParts = this.value.split("/");
                    $("#cldrCitiFromBirthday").datepicker('setDate', new Date(dateParts[2], dateParts[1] - 1, dateParts[0]));
                }
                if (this.value.length === 0){
                    $("#cldrCitiFromBirthday").datepicker('setDate', null);
                }
            }

        });

        $("#cldrCitiToBirthday").datepicker({autoclose: true, todayHighlight: true}).on('changeDate', function (e) {
                $(document).pathValue({
                    ngaySinhThanhVienDenNgay: e.format(0, "dd/mm/yyyy")
                });
            }
        );

        $('#ngaySinhThanhVienDenNgay').on('change', function () {
            var result =  $(fromThongTinHoNhanKhau).validation({
                checkBirthdate: {
                    ngaySinhThanhVienDenNgay: 'Ngày không đúng định dạng',
                }
            ...
            if (!result.isValid){
                ...
                if (this.value.length === 10) {
                    var dateParts = this.value.split("/");
                    $("#cldrCitiToBirthday").datepicker('setDate', new Date(dateParts[2], dateParts[1] - 1, dateParts[0]));
                }
                if (this.value.length === 0){
                    $("#cldrCitiToBirthday").datepicker('setDate', null);
                }
            }

        });

        /*tra cứu hộ cư trú trên địa bạn theo ngày sinh*/
        $("#srchFromBirthday").datepicker({autoclose: true, todayHighlight: true}).on('changeDate', function (e) {
                $(document).pathValue({
                    sch_ngaySinhChuHoTuNgay: e.format(0, "dd/mm/yyyy")
                });
            }
        );

        $('#sch_ngaySinhChuHoTuNgay').on('change', function () {
            var result =  $(formtraCuuHoCuTruNangCao).validation({
                checkBirthdate: {
                    sch_ngaySinhChuHoTuNgay: 'Ngày không đúng định dạng',
                }
            ...
            if (!result.isValid){
                ...
                if (this.value.length === 10) {
                    var dateParts = this.value.split("/");
                    $("#srchFromBirthday").datepicker('setDate', new Date(dateParts[2], dateParts[1] - 1, dateParts[0]));
                }
                if (this.value.length === 0){
                    $("#srchFromBirthday").datepicker('setDate', null);
                }
            }
        });
        ...
        $("#srchToBirthday").datepicker({autoclose: true, todayHighlight: true}).on('changeDate', function (e) {
                $(document).pathValue({
                    sch_ngaySinhChuHoDenNgay: e.format(0, "dd/mm/yyyy")
                });
            }
        );

        $('#sch_ngaySinhChuHoDenNgay').on('change', function () {
            var result =  $(formtraCuuHoCuTruNangCao).validation({
                checkBirthdate: {
                    sch_ngaySinhChuHoDenNgay: 'Ngày không đúng định dạng'
                }
            ...
            if (!result.isValid) {
               // $(formtraCuuHoCuTruNangCao).bindError(result);
                ...
                if (this.value.length === 10) {
                    var dateParts = this.value.split("/");
                    $("#srchToBirthday").datepicker('setDate', new Date(dateParts[2], dateParts[1] - 1, dateParts[0]));
                }
                if (this.value.length === 0){
                    $("#srchToBirthday").datepicker('setDate', null);
                }
            }

        });

        $("#srchCitiFromBirthday").datepicker({autoclose: true, todayHighlight: true}).on('changeDate', function (e) {
                $(document).pathValue({
                    sch_ngaySinhThanhVienTuNgay: e.format(0, "dd/mm/yyyy")
                });
            }
        );

        $('#sch_ngaySinhThanhVienTuNgay').on('change', function () {
            var result =  $(formtraCuuHoCuTruNangCao).validation({
                checkBirthdate: {
                    sch_ngaySinhThanhVienTuNgay: 'Ngày không đúng định dạng'
                }
            ...
            if (!result.isValid) {
                ...
                if (this.value.length === 10) {
                    var dateParts = this.value.split("/");
                    $("#srchCitiFromBirthday").datepicker('setDate', new Date(dateParts[2], dateParts[1] - 1, dateParts[0]));
                }
                if (this.value.length === 0){
                    $("#srchCitiFromBirthday").datepicker('setDate', null);
                }
            }
        });


        $("#srchCitiToBirthday").datepicker({autoclose: true, todayHighlight: true}).on('changeDate', function (e) {
                $(document).pathValue({
                    sch_ngaySinhThanhVienDenNgay: e.format(0, "dd/mm/yyyy")
                });
            }
        );

        $('#sch_ngaySinhThanhVienDenNgay').on('change', function () {
            var result =  $(formtraCuuHoCuTruNangCao).validation({
                checkBirthdate: {
                    sch_ngaySinhThanhVienDenNgay: 'Ngày không đúng định dạng'
                }
            ...
            if (!result.isValid) {
                ...
                if (this.value.length === 10) {
                    var dateParts = this.value.split("/");
                    $("#srchCitiToBirthday").datepicker('setDate', new Date(dateParts[2], dateParts[1] - 1, dateParts[0]));
                }
                if (this.value.length === 0){
                    $("#srchCitiToBirthday").datepicker('setDate', null);
                }
            }
        });

        /*Form tra cứu hộ tại địa bàn*/
        //select row
        $(document).on('click', '.radio', function ($event) {
            var $box = $(this);
            rowId.val($(this).attr('value'));
            finalHHRegBookProfNumber = $(this).attr("hhRegBookProfNumber");
            houseHoldResId = $(this).attr('houseHoldResId');
            if ($box.is(":checked")) {
        ...
        $(document).on('click', 'span.expand-collapse-icon-form1', function () {
            $(".box-legend").find('form').find('#timKiemTheoThongTinHoNhanKhau').animate({
                height: 'toggle'
            });
        });

        $(document).on('click', '.expand-collapse-icon-form1', function ($event) {
            if (isFormThongTinHoNhanKhau) {
                $($event.currentTarget).html('<i class="fa fa-chevron-circle-down text-primary" aria-hidden="true"></i> Tìm kiếm theo thông tin hộ, nhân khẩu');
            } else {
                $($event.currentTarget).html('<i class="fa fa-chevron-circle-up text-primary" aria-hidden="true"></i> Tìm kiếm theo thông tin hộ, nhân khẩu');
            }
            isFormThongTinHoNhanKhau = !isFormThongTinHoNhanKhau;
        });

        $(document).on('click', 'span.expand-collapse-icon-form2', function () {
            $(".box-legend").find('form').find('#thongTinBienDong').animate({
                height: 'toggle'
            });
        });

        $(document).on('click', '.expand-collapse-icon-form2', function ($event) {
            if (isFormThongTinNoiDen) {
                $($event.currentTarget).html('<i class="fa fa-chevron-circle-down text-primary" aria-hidden="true"></i> Tìm kiếm theo thông tin biến động cư trú');
            } else {
                $($event.currentTarget).html('<i class="fa fa-chevron-circle-up text-primary" aria-hidden="true"></i> Tìm kiếm theo thông tin biến động cư trú');
            }
            isFormThongTinNoiDen = !isFormThongTinNoiDen;
        });

        $(document).on('click', 'span.expand-collapse-icon-form3', function () {
            $(".modal-content").find('form').find('#traCuuHoCuTruTaiNoiThuongTru').animate({
                height: 'toggle'
            });
        });

        $(document).on('click', '.expand-collapse-icon-form3', function ($event) {
            if (isFormTraCuuHoCuTru) {
                $($event.currentTarget).html('<i class="fa fa-chevron-circle-down text-primary" aria-hidden="true"></i> Thông tin hộ');
            } else {
                $($event.currentTarget).html('<i class="fa fa-chevron-circle-up text-primary" aria-hidden="true"></i> Thông tin hộ');
            }
            isFormTraCuuHoCuTru = !isFormTraCuuHoCuTru;
        });

        /*Button event tra cứu hộ cư trú tại địa bàn*/
        $(formtraCuuHoCuTru).on('click', 'button[name="chonHoThemBienDong"]', function ($event) {
            traCuuHoCuTruTaiDiaBan('chon');
        });

        $(formtraCuuHoCuTru).on('click', 'button[name="quaylai"]', function ($event) {
            $("#sch_chinhXacTenChuHo").prop("checked", false);
            $("#sch_ChinhXacTenThanhVien").prop("checked", false);
            $("#thongTinHoKhau").empty();
            $(formtraCuuHoCuTru).hideDialog();
        });

        /*Xử lý logic form tra cứu biến động hộ*/
        $(formThongTinBienDong).on('change', 'select[name="movingType"]', function ($event) {
            switch ($(this).val()) {
                case "0":

                    $('select[name=regPlaceCityId]').attr("data-param",
                        JSON.stringify({
                            showAll: true
                        }));
                    $('select[name=regPlaceDistrictId]').attr("data-param",
                        JSON.stringify({
                            showAll: true
                        }));
                    $('select[name=regPlaceVillageId]').attr("data-param",
                        JSON.stringify({
                            showAll: true
                        }));
                    $(formThongTinBienDong).pathValue({
                        regPlaceCityId: null,
                        regPlaceDistrictId: null,
                        regPlaceVillageId: null
                    });

                    $(formThongTinBienDong).disableControlValue({
                        regPlaceCityId: false,
                        regPlaceDistrictId: true,
                        regPlaceVillageId: true
                    });
                    break;
                case "1":
                ...
                case "3":
                    ...
                    $(formThongTinBienDong).pathValue({
                        regPlaceCityId: userCityId,
                        regPlaceDistrictId: userDistrictId,
                        regPlaceVillageId: null
                    });
                    $(formThongTinBienDong).disableControlValue({
                        regPlaceCityId: true,
                        regPlaceDistrictId: true,
                        regPlaceVillageId: false
                    });
                    break;
                case "4":
                    $('select[name=regPlaceCityId]').attr("data-param",
                    ...
                    $(formThongTinBienDong).pathValue({
                        regPlaceCityId: userCityId,
                        regPlaceDistrictId: userDistrictId,
                        regPlaceVillageId: userVillageId
                    });
                    $(formThongTinBienDong).disableControlValue({
                        regPlaceCityId: true,
                        regPlaceDistrictId: true,
        ...
        //--Event-- Form form_asHouseHold
        /*Xử lý logic form thêm mới biến động*/
        $('input[type=radio][name=destionalType]').on('change', function () {
            switch ($(this).val()) {
                case "1":
                    $("#divMovingCountryId").hide();
                    $("#divMovingType").show();
                    $("#divMovingInCountry").show();
                    $("#divMovingAddress").show();
                    /*fixbug khi đi ngoaì nước hoặc không rõ nơi cư trú => chọn lại trong nước*/
                    $(formId).disableControlValue({
                        movingCityId: false,
                        movingDistrictId: true,
                        movingVillageId: true
        ...
        $('select[name=placeToMovingType]').on('select2:select', function ($event) {
            switch ($(this).val()) {
                case "1":
                ...
                case "3":
                    ...
                    $(formId).pathValue({
                        movingCityId: userCityId,
                        movingDistrictId: userDistrictId,
                        movingVillageId: null
                    });
        ...
        $('select[name=movingCityId]').on('select2:select', function ($event) {
            changeMovingCity(this);
        });
        $('select[name=movingCityId]').on('select2:clear', function ($event) {
            changeMovingCity(this);
        });

        function changeMovingCity(outside) {
            var movingCityId = $(outside).val();
            if (movingCityId != null) {
                $(formId).disableControlValue({
                    movingDistrictId: false
                });
                $('select[name=movingDistrictId]').attr("data-param",
        ...
        $('select[name=movingDistrictId]').on('select2:select', function ($event) {
            changeMovingDistrict(this);
        });
        $('select[name=movingDistrictId]').on('select2:clear', function ($event) {
            changeMovingDistrict(this);
        });

        function changeMovingDistrict(outside) {
            var movingDistrictId = $(outside).val();
            if (movingDistrictId !== null) {
                $(formId).disableControlValue({
                    movingVillageId: false
                });
                $('select[name=movingVillageId]').attr("data-param",
                    JSON.stringify({
                        districtId: movingDistrictId,
                        showAll: true
                    }));
            } else {
                $(formId).disableControlValue({
        ...
        $(formId).on('change', 'input[name="movingToDate"]', function ($event) {
            var movingToDate = $(this).val();
            if (movingToDate != "" && lib.parsetoDate(movingToDate, "/") < lib.parsetoDate(ngayHienTai, "/")) {
                $(formId).pathValue({
                    movingStatus: "1"
                });
            } else if (movingToDate != "" && lib.parsetoDate(movingToDate, "/") >= lib.parsetoDate(ngayHienTai, "/")) {
                $(formId).pathValue({
                    movingStatus: "2"
                });
            }
        ...
        $('select[name=movingStatus]').on('select2:select', function ($event) {
            $(formId).pathValue({
                movingToDate: ''
            })
        });

        $('select[name="statusReceive"]').on('select2:select', function (e) {
            var statusReceive = $(this).val();
            statusReceiveChange(statusReceive);
        });

        // $('select[name="sourceData"]').on('select2:select', function (e) {
        //     var sourceData = $(this).val();
        //     sourceDataChange(sourceData);
        // });

        function statusReceiveChange(val) {
            if (val == KEY_DA_TAO_BIEN_DONG){
                $(document).disableControlValue({movingStatus: false});
                $(advSearchId).disableControlValue({movingStatus: false});
            }else {
                $(document).pathValue({movingStatus: "0"});
                $(document).disableControlValue({movingStatus: true});
                $(advSearchId).pathValue({movingStatus: "0"});
                $(advSearchId).disableControlValue({movingStatus: true});
            }

        }

        // function sourceDataChange(val) {
        //     if (val == -1|| val == 0) {
        //         $(document).pathValue({statusReceive: "3"});
        //         $(advSearchId).pathValue({statusReceive: "3"});
        //         $(document).disableControlValue({
        //             statusReceive: true,
        //             movingStatus: false
        //         });
        //         $(advSearchId).disableControlValue({
        ...
        //         });
        //     } else {
        //         $(document).disableControlValue({statusReceive: false});
        //         $(advSearchId).disableControlValue({statusReceive: false});
        //     }
        // }

        /*fil thông tin hộ khẩu*/
        $('input[name=hhRegBookProfNumber]').keydown(function (e) {
            if (e.keyCode == 13 || e.keyCode == 9) {
                $(formId).resetValidation();
                houseHoldResId = null;
                finalHHRegBookProfNumber = $(this).val();
                actions = 'chon';
                dataNextPage();
            }
        ...
        $(formId).on('click', 'button[name="btn-ketThucBienDong"]', function ($event) {
            var id = $('#PageId').val();
            capNhatVeSom('capnhatngayve', id);
        });

        $(formId).on('click', 'button[name="btn-ghivathem"]', function ($event) {
            var id = $('#PageId').val();
            var jsonData = $(formId).getValue();
            formDialogCrud('ghivathem', id !== '' ? id : null, false, '' , KEY_DA_TAO_BIEN_DONG, KEY_CHECK_HK15);
        });

        $(formId).on('click', 'button[name="btn-ghi"]', function ($event) {
            var id = $('#PageId').val();
            if (id !== '') {
                formDialogCrud('ghi', id !== '' ? id : null, false, '', KEY_DA_TAO_BIEN_DONG, KEY_CHECK_HK15);
            } else {
                formDialogCrud('ghi', id !== '' ? id : null, false,'', KEY_DA_TAO_BIEN_DONG, KEY_CHECK_HK15);
            }
        });

        $(formId).on('click', 'button[name="btn-khongtaobiendong"]', function ($event) {
            var id = $('#PageId').val();
            if (id !== '') {
                formDialogCrud('ghi', id !== '' ? id : null, false, '', KEY_KHONG_TAO_BIEN_DONG, KEY_EXIT_HK15);
            }
        });

        $(formId).on('click', 'button[name="btn-taobiendong"]', function ($event) {
            var id = $('#PageId').val();
            if (id !== '') {
                formDialogCrud('ghi', id !== '' ? id : null, false, '', KEY_CHO_TAO_BIEN_DONG, KEY_CHECK_HK15);
            }
        });


        //button close form
        $(formId).on('click', 'button[name="btn-close"]', function ($event) {
            var id = $('#PageId').val();
            if (jsonFormId !== JSON.stringify($(formId).getValue())) {
                if (id != '' && (actionDialog === 'xem' || actionDialog === 'capnhatngayve')) {
                    listArrayRowSel = [];
                    $(formId).hideDialog();
                    $(formId).resetValidation();
                } else if (id != '' && actionDialog === 'sua') {
                    lib.confirm('Dữ liệu đã nhập sẽ không thay đổi',
                        function (rs) {
                            if (!rs) {
                                return;
                            }
                    ...
                    lib.confirm('Dữ liệu đã nhập sẽ không được thêm mới',
                        function (rs) {
                            if (!rs) {
                                return;
                            }
        ...
        /*Form kết thúc biến động hộ*/
        //button xác nhận biến động của hộ
        $(formKetThucBienDong).on('click', 'button[name="btn-xacNhanKetThuc"]', function ($event) {
            var id = $('#PageId').val();
            var formData = $(formId).getValue();
            var ngayKetThucBienDong = $("#movingToDateEnd").val();
            var result = $(formKetThucBienDong).validation({
                required:{
                    movingToDateEnd: 'Ngày kết thúc biến động không được trống'
                },
                checkFullDate: {
                    movingToDateEnd: 'Ngày kết thúc biến động không đúng định dạng'
                }
            });
            if (!result.isValid){
                $(formKetThucBienDong).bindError(result);
                return;
            }
            if (lib.parsetoDate(ngayKetThucBienDong, "/") <= lib.parsetoDate(ngayHienTai, "/")){
                capNhatVeSom('xacNhan', id, formData.movingFromDate, ngayKetThucBienDong, KEY_CHECK_HK15);
            }else {
                lib.showMessage('Ngày kết thúc không được lớn hơn ngày hiện tại',
                    'error', function () {
                        // lib.reload();
                    });
            }
        ...
        //button hủy xác nhận kết thúc biến động của hộ
        $(formKetThucBienDong).on('click', 'button[name="btn-huyXacNhan"]', function ($event) {
            $(formKetThucBienDong).hideDialog();
        });
        //--------------------------------------------------------------------------------
        //--------------------------------------------------------------------------------
        $("#btn-xacNhanXoaBienDongHo").on('click', function () {
            var id =new Array($('#PageId').val());
            var formData = $(formId).getValue();
            let listNameNK = $(formXoaBienDong).find('td[class="col-ho-ten"]').toArray().map(item => item.innerText.trim());
            var ngayKetThucBienDong = $("#movingToDateEnd").val();
            xoaBienDongHo('xacNhan', id, listNameNK.join(', '), formData.movingFromDate, ngayKetThucBienDong, KEY_CHECK_HK15);
            // var result = $(formXoaBienDong).validation({
            //     required:{
            //         movingToDateEnd: 'Ngày kết thúc biến động không được trống'
            //     },
            //     checkFullDate: {
            ...
            // }
            // if (lib.parsetoDate(ngayKetThucBienDong, "/") <= lib.parsetoDate(ngayHienTai, "/")){
            //     xoaBienDongHo('xacNhan', id, formData.movingFromDate, ngayKetThucBienDong, KEY_CHECK_HK15);
            // }else {
            //     lib.showMessage('Ngày kết thúc không được lớn hơn ngày hiện tại',
            //         'error', function () {
            //             // lib.reload();
